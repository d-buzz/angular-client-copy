...
@Component({
  selector: 'app-view-resaids',
  templateUrl: './view-resaids.component.html',
  styleUrls: ['./view-resaids.component.scss']
})
export class ViewResaidsComponent implements OnInit {
  @Input() data;
  ...
  mapAuthorAccounts(){
    this.showing = true;
    let voters = { usernames: JSON.stringify(this.data) };
      ...
      if(res.status === 200 && res.data){
        ...
        res.data.forEach(info => {
          ...
          this._auth.isFollowing(info.name, this.auth_username).subscribe((res1: any) => {
            ...
            this.mapped_rebloggers.push(account);
            ind++;
  ...
  loadData(limit){
    this.rebloggers = this.mapped_rebloggers.slice(this.offset,limit);
